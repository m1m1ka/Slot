...
* **头奖（Jackpot）**：一张刮刮卡当前可以刮出的最高分图案就是头奖。实现上以该卡本次生成使用的有效图案池中 `BaseScore` 最高的图案为准；如果多个图案并列最高，它们都属于头奖图案。
* **头奖概率**：指最终呈现在卡面上的可刮格中，至少出现一个头奖图案的概率。它不是单格抽取权重，也不是简单修改图案权重。
* **头奖概率类肉鸽效果**：必须在卡面正常生成后做概率补正。若自然生成已出现头奖，不再额外处理；若未出现头奖，则按补正概率把一个可刮格替换为头奖图案，从而使最终卡面“至少出现一个头奖”的概率达到目标值。
