...
namespace Core
{
    public class MetalMatchAnyPairSettlementEvaluator : IScratchSettlementEvaluator
    {
        private const double MatchScoreMultiplier = 1d;
        private const int RequiredMatchCount = 2;

        private static readonly HashSet<int> MetalPatternIds = BuildMetalPatternIds();
        ...
        public ScratchSettlementResult Evaluate(ScratchCardModel model)
        {
            ...
            Dictionary<int, List<ScratchCellModel>> cellsByPattern = BuildMetalCellsByPattern(model);
            ...
            return new ScratchSettlementResult
            {
                ...
                Summary = score > 0 ? "Two or more matching metal patterns scored." : "No matching metal pattern pair.",
        ...
        private static Dictionary<int, List<ScratchCellModel>> BuildMetalCellsByPattern(ScratchCardModel model)
        {
            ...
            for (int i = 0; i < model.Cells.Count; i++)
            {
                ...
                if (cell == null ||
                    !cell.IsScratchable ||
                    !cell.IsScratched ||
                    !MetalPatternIds.Contains(cell.PatternId) ||
        ...
        private static HashSet<int> BuildMetalPatternIds()
        {
            var patternIds = new HashSet<int>();
            ScratchPatternPoolConfig metalPool = ScratchCardDefaultsProvider.GetPatternPool(ScratchCardDefaultsProvider.MetalPatternPoolId);
            if (metalPool?.Entries == null)
            {
                return patternIds;
            }

            for (int i = 0; i < metalPool.Entries.Count; i++)
            {
                ScratchPatternPoolEntryConfig entry = metalPool.Entries[i];
